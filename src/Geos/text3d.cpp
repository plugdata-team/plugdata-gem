...
void text3d :: setFontSize()
{
  ...
  if (m_pyfont) {
    if (! m_pyfont->FaceSize(fs) ) {
      pd_error(nullptr, "unable to set fontsize!");
    }
    if(m_pyfont->Error()) {
      pd_error(nullptr, "error setting fontsize");
    }
  }
  if (m_aafont) {
    if (! m_aafont->FaceSize(fs) ) {
      pd_error(nullptr, "unable to set antialiased-fontfize!");
    }
    if(m_aafont->Error()) {
      pd_error(nullptr, "error setting aa-fontsize");
    }
  }
  m_font=selectFont();
