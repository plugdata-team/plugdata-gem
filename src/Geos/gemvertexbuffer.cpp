...
failed:
  pd_error(nullptr, "illegal arguments to '%s': must be <table[1..%d]> [<offset>]",
        name.c_str(), vb.dimen);
  return;
}
...
void gemvertexbuffer :: enableMess (t_symbol*s, int argc, t_atom *argv)
{
  ...
  for(i=0; i<argc; i++) {
    if(0) {
      ...
      pd_error(nullptr, "enable: illegal argument#%d: must be 'position', 'color', 'texture', 'normal' or 'attribute'",
            i);
    }
  }
}
void gemvertexbuffer :: disableMess (t_symbol*s, int argc, t_atom *argv)
{
  ...
  for(i=0; i<argc; i++) {
    if(0) {
      ...
      pd_error(nullptr, "disable: illegal argument#%d: must be 'position', 'color', 'texture', 'normal' or 'attribute'",
            i);
    }
  }
}
                                ...
                                gem::VertexBuffer&array, unsigned int offset)
{
  ...
  if ( argv[0].a_type != A_SYMBOL ) {
    pd_error(nullptr, "first arg must be symbol (table name)");
    return;
  }
  if ( argc > 1 ) {
    if ( argv[1].a_type != A_FLOAT ) {
      pd_error(nullptr, "second arg must be float (offset)");
    } else {
                                  ...
                                  bool resize)
{
  ...
  if(offset>vb.size) {
    pd_error(nullptr, "offset %d is bigger than vertexbuffer size (%d) for %s", offset, vb.size, tab_name.c_str());
    return;
  }
  ...
  if (!(a = (t_garray *)pd_findbyclass(s, garray_class))) {
    pd_error(nullptr, "%s: no such array", tab_name.c_str());
    return;
  }
  if (!garray_getfloatwords(a, &npoints_, &vec)) {
    pd_error(nullptr, "%s: bad template for tabLink", tab_name.c_str());
    return;
  }
  if(npoints_<0) {
    pd_error(nullptr, "%s: illegal number of elements %d", tab_name.c_str(), npoints_);
  }
  npoints=npoints_;
...
void gemvertexbuffer :: attribute(t_symbol*s, int argc, t_atom *argv)
{
  ...
  if(glsl_program==0) {
    pd_error(nullptr, "glsl_program has not been set");
    return;
  }

  if((argc!=2 && argc!=3) || (argv[0].a_type!=A_SYMBOL
                              || argv[1].a_type!=A_SYMBOL)) {
    pd_error(nullptr, "illegal arguments to 'attribute': must be <vbo_index> <attribute_name> <table> [<offset>]");
    return;
  }
  if(argc==3) {
  ...
  if(glsl_index==-1) {
    pd_error(nullptr, "illegal arguments to 'attribute': '%s' not found in glsl_program",
          name_ch);
    return;
  }
  ...
  case GL_FLOAT_MAT4:
    ...
    break;
  default:
    pd_error(nullptr, "illegal arguments to 'attribute': must be type 'float', 'vec2-4' or 'mat2-4'");
    return;
  }
