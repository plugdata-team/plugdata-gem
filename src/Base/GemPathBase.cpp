    ...
    m_out1(NULL)
{
  ...
  if (argc >= 1) {
    ...
    if (m_numDimens > 64) {
      pd_error(nullptr, "too many dimensions, must be below 64");
      m_numDimens = 64;
    }
  }
}
...
void GemPathBase :: openMess(t_symbol *arrayname)
{
  ...
  if (!m_array) {
    pd_error(nullptr, "unable to find array %s", arrayname->s_name);
    return;
  }
}
void GemPathBase :: floatMess(t_float val)
{
  ...
  if (size % m_numDimens) {
    pd_error(nullptr, "size is not a mod of dimensions");
    return;
  }
