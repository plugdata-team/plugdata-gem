...
bool vertex_program :: isRunnable()
{
  ...
  pd_error(nullptr, "need ARB (or NV) vertex_program extension for shaders");
  return false;
}
...
void vertex_program :: openMess(t_symbol *filename)
{
  ...
  if(file) {
    ...
    if(size<0) {
      fclose(file);
      pd_error(nullptr, "error reading filesize");
      return;
    }
    ...
    fclose(file);
    if(err) {
      pd_error(nullptr, "error %d reading file (%d<%d)", err, count, size);
      return;
    }
  } else {
...
void vertex_program :: LoadProgram(void)
{
  ...
  if((GEM_PROGRAM_NV == m_programType) && (!GLEW_NV_vertex_program)) {
    pd_error(nullptr, "NV vertex programs not supported by this system");
    return;
  }

  if((GEM_PROGRAM_ARB == m_programType) && (!GLEW_ARB_vertex_program)) {
    pd_error(nullptr, "ARB vertex programs not supported by this system");
    return;
  }
  ...
  if(err != -1) {
    ...
    *e = '\0';
    pd_error(nullptr, "program error at line %d:\n\"%s\"\n",line,s);
  ...
  if(GLEW_ARB_vertex_program) {
    ...
    if (isUnderNativeLimits!=1) {
      ...
      pd_error(nullptr, "is beyond hardware limits");
...
void vertex_program :: startRendering()
{
  if (m_programString == NULL) {
    pd_error(nullptr, "need to load a program");
    return;
  }
