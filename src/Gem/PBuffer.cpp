  ...
  , data(NULL)
{
  ...
  try {
    ...
    pd_error(nullptr, "PBuffer::PBuffer(): %s",err);
...
void PBuffer::enable()
{
  ...
  if(!glXMakeCurrent(data->display,data->pbuffer,data->context)) {
    pd_error(nullptr, "PBuffer::enable(): glXMakeCurrent() failed");
  }
}
...
void PBuffer::disable()
{
  if(!glXMakeCurrent(data->display,data->old_pbuffer,data->old_context)) {
    pd_error(nullptr, "PBuffer::disable(): glXMakeCurrent() failed");
  }
}
...
void reportError (char * strError)
{
  ...
  pd_error(nullptr, "Error: %s", strError);
}
  ...
  height(height)
{
  ...
  try {
    ...
    pd_error(nullptr, "GemPBuffer: %s",err);
    hdc = old_hdc;
    context = old_context;
  }
...
void PBuffer::enable()
{
  ...
  if(!wglMakeCurrent(data->hdc,data->context)) {
    pd_error(nullptr, "PBuffer::disable(): wglMakeCurrent() failed");
  }
}
...
PBuffer::PBuffer(int width,int height,int flags)
  ...
  , data(nullptr)
{
  pd_error(nullptr, "PBuffer::PBuffer(): no PBuffer support");
}
...
void PBuffer::enable()
{
  pd_error(nullptr, "PBuffer::enable(): no PBuffer support");
}

void PBuffer::disable()
{
  pd_error(nullptr, "PBuffer::disable(): no PBuffer support");
}
