...
GEM_EXTERN unsigned char* imageStruct::allocate(size_t size)
{
  ...
  try {
    ...
    pd_error(nullptr, "out of memory!");
    data=pdata=NULL;
    datasize=0;
    return NULL;
  }
...
{
  if (!to || !data) {
    pd_error(nullptr, "GEM: Someone sent a bogus pointer to copy2ImageStruct");
    if (to) {
      to->data = NULL;
    }
    return;
...
GEM_EXTERN void imageStruct::copy2Image(imageStruct *to) const
{
  if(!copy_imagestruct2imagestruct(this, to)) {
    pd_error(nullptr, "GEM: Someone sent a bogus pointer to copy2Image");
    if (to) {
      to->data = NULL;
    }
    return;
...
{
  if (!to || !data) {
    pd_error(nullptr, "GEM: Someone sent a bogus pointer to refreshImage");
    return;
  }
...
{
  if (!from || !from->data) {
    pd_error(nullptr, "GEM: Someone sent a bogus pointer to convert");
    return false;
  }
  xsize=from->xsize;
  ysize=from->ysize;
  ...
  switch (from->format) {
  default:
    pd_error(nullptr, "%s: unable to convert from %d", __FUNCTION__, from->format);
    break;
  case GL_RGBA:
...
GEM_EXTERN bool imageStruct::convertTo(imageStruct *to, unsigned int fmt) const
{
  ...
  switch (format) {
  default:
    pd_error(nullptr, "%s: unable to convert to %d", __FUNCTION__, format);
    break;
  case GL_RGBA:
                                    ...
                                    unsigned char*u, unsigned char*v) const
{
  ...
  case GL_RGB:
  case GL_BGR_EXT:
    pd_error(nullptr, "getYUV not implemented for RGB");
    return false;
  case GL_RGBA:
  case GL_BGRA_EXT:
    pd_error(nullptr, "getYUV not implemented for RGBA");
    return false;
  case GL_YUV422_GEM:
    position = (((X+(upsidedown?(ysize-Y-1):Y)*xsize)>>1)<<1);
    pixels=data+position*csize;
