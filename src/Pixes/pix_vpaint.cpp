...
bool pix_vpaint :: isRunnable(void)
{
  ...
  */
  if(!GLEW_VERSION_1_2) {
    pd_error(nullptr, "openGL-1.2 support missing");
    return false;
  }

  if(!GLEW_EXT_blend_minmax && !GLEW_SGI_color_matrix) {
    pd_error(nullptr, "both color_matrix and blend_minmax extension missing");
    return false;
  }
...
void pix_vpaint :: init()
{
  ...
  */
  ...
  if (canDrawEdges) {
    ...
    pd_error(nullptr, "This OpenGL implementation does not support the color matrix and/or\n");
    pd_error(nullptr, "the min/max blending equations, therefore the option to draw the\n");
    pd_error(nullptr, "Voronoi region edges is unavailable.\n\n");
    pd_error(nullptr, "The required features are available with OpenGL 1.2 or the GL_EXT_blend_minmax\n");
    pd_error(nullptr, "and GL_SGI_color_matrix extensions.\n");
  }
  m_initialized = 1;
  m_pbuffer->disable();
}
