...
void pix_background :: processYUVAltivec(imageStruct &image)
{
  ...
  if (image.ysize*image.xsize % 16 != 0) {
    pd_error(nullptr, "image not properly aligned for Altivec - try something SD or HD maybe?");
    return;
  }
  ...
  if (m_reset) {
   ...
   vector unsigned int                        Uhi, Ulo, Vhi, Vlo,
           Ures,Vres;
   vector bool int                    Umasklo, Umaskhi, Vmaskhi,
           Vmasklo;
...
void pix_background :: rangeNMess(int argc, t_atom*argv)
{
  ...
  case 1:
    ...
    break;
  default:
    pd_error(nullptr, "only 1 or 3 values are allowed as ranges (not %d)", argc);
  }
}
