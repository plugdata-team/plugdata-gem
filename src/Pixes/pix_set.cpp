...
void pix_set :: FILLMess(t_symbol *s, int argc, t_atom *argv)
{
  ...
  case GL_RGB:
    if ( argc==1 ) {
      ...
      pd_error(nullptr, "fill need 1 or 3 float arg in RGB mode");
      return;
    }
    while (counter--) {
      buffer[chRed]   = r; // red
  ...
  case GEM_GRAY:
    if ( argc>0 ) {
      ...
      pd_error(nullptr, "fill need 1 float arg in GREY mode");
      return;
    }
    while (counter--) {
      buffer[chRed] = r;
  ...
  default:
    if ( argc==1 ) {
      ...
      pd_error(nullptr, "fill need 1 or 4 float arg in RGBA mode");
      return;
    }
    while (counter--) {
      buffer[chRed]   = r; // red
