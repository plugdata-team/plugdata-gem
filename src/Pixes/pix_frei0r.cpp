...
public:
  bool init(void)
  {
    ...
    if (err<0) {
        pd_error(nullptr, "[pix_frei0r] failed to initialize plugin");
      return false;
    }
    ...
    case (F0R_PLUGIN_TYPE_FILTER):
      break;
    default:
      pd_error(nullptr, "[pix_frei0r] only supports sources/filters, no mixers!");
      return false;
    }
...
# warning check compatibility
  ...
  void close(void)
  {
    ...
    if(f0r_deinit) {
      int err=f0r_deinit();
      if(err) {
          pd_error(nullptr, "[%s] f0r_deinit() failed with %d", m_name.c_str(), err);
      }
    }
  }
