...
{
  if (f<0) {
    pd_error(nullptr, "frame# must not be less than zero (%d)", f);
  }
  m_frame=f;
}
/////////////////////////////////////////////////////////
  ...
  if (state && img && &img->image) {
    if (img->newimage || m_frame!=m_lastframe) {
      if(m_bindname==NULL || m_bindname->s_name==NULL) {
        pd_error(nullptr, "cowardly refusing to write to no pix_buffer");
        m_frame=-1;
        return;
      }
      Obj_header*ohead=(Obj_header*)pd_findbyclass(m_bindname, pix_buffer_class);
      if(ohead==NULL) {
        pd_error(nullptr, "couldn't find pix_buffer '%s'", m_bindname->s_name);
        m_frame=-1;
        return;
      }
