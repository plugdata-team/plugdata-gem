...
# endif
    ...
    if ((fd=canvas_open(const_cast<t_canvas*>(canvas), name.c_str(), extension,
      ...
      if(canvas) {
        ...
        if(loud) {
            pd_error(nullptr, "pix_freeframe[%s]: unfindeable", name.c_str());
        }
        return false;
      }
    }
    ...
    if(!m_dlhandle) {
      if(loud) {
          pd_error(nullptr, "pix_freeframe[%s]: %s", libname.c_str(), dlerror());
      }
      return NULL;
    }
    dlerror();
...
{
  if(!m_plugin) {
    pd_error(nullptr, "no instance of plugin available");
    return;
  }
  if(key.empty()) {
    pd_error(nullptr, "unknown key '%s'", key.c_str());
    return;
  }
