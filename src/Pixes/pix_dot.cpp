...
void pix_dot :: sizeMess(int width, int height)
{
  if(width>0) {
    ...
    pd_error(0, "width must be > 0!");
  }
  if(height>0) {
    dots_height=height;
  } else {
    pd_error(0, "height must be > 0!");
  }
  m_useScale=false;
  alreadyInit=0;
  myImage.setBlack();
...
void pix_dot :: processRGBAImage(imageStruct &image)
{
  ...
  if (!alreadyInit)    {
    ...
    if (pattern == NULL) {
      pd_error(0, "couldn't make RGBA pattern");
      return;
    }
...
void pix_dot :: processYUVImage(imageStruct &image)
{
  ...
  if (!alreadyInit) {
    ...
    pattern = (U32 *)malloc(DOTMAX * dot_hsize * dot_hsize * sizeof(U32));
    if (pattern == NULL) {
      pd_error(0, "couldn't make YUV pattern");
      return;
    }
...
void pix_dot :: processGrayImage(imageStruct &image)
{
  ...
  if (!alreadyInit) {
    ...
    pattern = (U32 *)malloc(DOTMAX * dot_hsize * dot_hsize * sizeof(U32));
    if (pattern == NULL) {
      pd_error(0, "couldn't make luma pattern");
      return;
    }
...
{
  if(state<=0.f) {
    pd_error(0, "scale-factor must not be < 0!");
    return;
  }
